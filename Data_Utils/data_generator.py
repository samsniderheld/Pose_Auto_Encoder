...
#     'Generates data for tf.keras'
    ...
    def __data_generation(self, idx):
        ...
        X_csv = np.empty((self.batch_size,52,2))
            ...
            with open(batch_file[1]) as file:
                ...
                for j,row in enumerate(csv_reader):
                    for k, val in enumerate(row[1:3]):
                      X_csv[i,j,k] = float(val)/1024
            
            
        
        return ([X_img, X_csv],)

class BoneDataGeneratorWithWeight(tf.keras.utils.Sequence):
#     'Generates data for tf.keras'
    def __init__(self, args, shuffle=True,):
        self.shuffle = shuffle
        self.img_dir = os.path.join(args.base_data_dir,args.input_data_dir_img + "*")
    ...
    def __data_generation(self, idx):
        ...
        X_csv_weight = np.empty((self.batch_size,52,3))
            ...
            with open(batch_file[1]) as file:
                ...
                for j,row in enumerate(csv_reader):
                    for k, val in enumerate(row[4:7]):
                      ...
                      X_csv_weight[i,j,k] = float(row[7])
